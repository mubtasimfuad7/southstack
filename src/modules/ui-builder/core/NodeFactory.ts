...
export class NodeFactory {
  ...
  static createFrame(id: string, name: string): DesignNode {
    return {
      ...
      decorators: [
        ...
        { id: `dec-${Date.now()}-ta`, type: 'text-align', config: { align: 'left', verticalAlign: 'top' }, enabled: true }
      ],
      children: [],
      attributes: {}
    };
  ...
  static createText(id: string, name: string, content: string): DesignNode {
    return {
      ...
      decorators: [
        ...
        { id: `dec-${Date.now()}-style`, type: 'text-style', config: { fontSize: 16, fontWeight: '400', fontFamily: 'Inter' }, enabled: true },
  ...
  static createGroup(id: string, name: string): DesignNode {
    return {
      ...
      rotation: 0,
      decorators: [
        { id: `dec-${Date.now()}-g1`, type: 'layout', config: { x: 0, y: 0, width: 200, height: 200 }, enabled: true },
        { id: `dec-${Date.now()}-ta`, type: 'text-align', config: { align: 'left', verticalAlign: 'top' }, enabled: true },
        { id: `dec-${Date.now()}-g2`, type: 'style', config: { opacity: 100 }, enabled: true }
      ],
      children: [],
      attributes: {}
    };
