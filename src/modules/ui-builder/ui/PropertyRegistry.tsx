    ...
    <div className="p-4 space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <PropInput label="X" value={node.x} onChange={(v: any) => updateNode(node.id, { x: v })} />
          <PropInput label="Y" value={node.y} onChange={(v: any) => updateNode(node.id, { y: v })} />
          <PropInput label="W" value={node.width} onChange={(v: any) => updateNode(node.id, { width: v })} />
          <PropInput label="H" value={node.height} onChange={(v: any) => updateNode(node.id, { height: v })} />
        </div>
      </section>
